...
1. Scan every frame for hazards in the vehicle's path or immediate surroundings.
2. For each hazard, estimate WHEN in the clip it first becomes visible (as seconds from chunk start).
3. Estimate WHERE in the frame the hazard is located (as approximate bounding box).
4. Assess severity based on potential impact to a vehicle traveling at normal speed.
